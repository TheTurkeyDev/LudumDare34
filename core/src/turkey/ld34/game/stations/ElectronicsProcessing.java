...
public class ElectronicsProcessing extends StationBase
{
	private int plasticStored = 0;
	private int metalStored = 0;
	private int circuitryStored = 0;
	private int plasticUsedPer = 2;
	private int metalUsedPer = 2;
	private int circuitryUsedPer = 3;
	...
	public ElectronicsProcessing(Vector2 location)
	{
		super(location, 64, 64, "stations/internalProcessing.png", "Electronics Processing");
		super.cost = 10;
		super.powerNeeded = 2;
	}
	...
	public void renderGuiInfo(Screen screen)
	{
		...
		Draw2D.drawString(125, 300, "Electronics Stored: ", 3, Color.WHITE);
		...
		String electronicsS = "" + this.electronicsStored;
	...
	public void update()
	{
		...
		if(this.processTimeLeft == 0)
		{
			this.electronicsStored++;
		}

		if(this.processTimeLeft <= 0 && this.plasticStored >= this.plasticUsedPer && this.metalStored >= this.metalUsedPer && this.circuitryStored >= this.circuitryUsedPer)
		{
	...
	public void onVisitByWorker(Entity entity)
	{
		...
		{
			this.plasticStored += ((Worker) entity).takeResource("Raw Plastic", this.plasticUsedPer);
			this.metalStored += ((Worker) entity).takeResource("Raw Metal", this.metalUsedPer);
			this.circuitryStored += ((Worker) entity).takeResource("Raw Circuitry", this.circuitryUsedPer);
		}
		else if(entity.getAction() != null && entity.getAction() instanceof ActionCollection)
		{
			if(this.electronicsStored > 0)
			{
				this.electronicsStored--;
				((Worker) entity).giveResource("Electronics", 1);
			}
		}

	}

	public int getElectronicsStored()
	{
		return this.electronicsStored;
	}
}